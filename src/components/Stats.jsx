...
function SortTh({ label, sortKey, sortState, onSort, className }) {
  const [activeKey, dir] = sortState;
  const active = activeKey === sortKey;
  return (
    <th className={className} onClick={() => onSort(sortKey)} style={{ cursor: 'pointer', color: active ? 'var(--accent)' : undefined }}>
          ...
          <table>
            ...
            <tbody>
              {rows.map(({ p, team, stats }) => (
                <tr key={p.id} style={team.id === league.userTeamId ? { background: 'var(--panel2)' } : {}}>
    ...
    <div className="panel">
      <div style={{ overflowX: 'auto' }}>
        <table>
          ...
          <tbody>
            {sorted.map((r) => (
              <tr key={r.team.id} style={r.team.id === league.userTeamId ? { background: 'var(--panel2)' } : {}}>
...
const LEADER_CATS = [
  ['Points', 'PPG', (s) => perGame(s, 'pts')],
  ['Rebounds', 'RPG', (s) => perGame(s, 'reb')],
  ['Assists', 'APG', (s) => perGame(s, 'ast')],
  ['Steals', 'SPG', (s) => perGame(s, 'stl')],
  ['Blocks', 'BPG', (s) => perGame(s, 'blk')],
  ['Field Goal %', 'FG%', (s) => fgPct(s) * 100],
  ['3-Point %', '3P%', (s) => tpPct(s) * 100],
  ['True Shooting %', 'TS%', (s) => tsPct(s) * 100],
];
...
function LeagueLeadersTab({ league, openPlayer, openTeam }) {
  ...
  return (
    <div>
      ...
      <div className="grid2">
        {LEADER_CATS.map(([label, unit, valueFn]) => {
